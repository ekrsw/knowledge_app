...
- [ ] テスト用フィクスチャの拡充（ユーザー、記事、修正案のサンプルデータ）
- [ ] テスト用データベースの初期化スクリプト作成
- [ ] 認証トークン生成ヘルパー関数の実装
- [ ] テストユーティリティ関数の作成
...
- [ ] UserFactory（user、approver、admin各ロール）
- [ ] ApprovalGroupFactory（複数グループのサンプル）
- [ ] ArticleFactory（様々なカテゴリ・内容の記事）
- [ ] RevisionFactory（各ステータスの修正案）
- [ ] NotificationFactory（通知サンプル）
