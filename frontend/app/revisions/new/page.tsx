...
import { InfoCategory, Article, CreateRevisionRequest } from '@/types/revision'
...
function NewRevisionContent() {
  ...
  const handleSubmit = async (isDraft: boolean = true) => {
    ...
    try {
      const data: CreateRevisionRequest = {
        title: formData.title.trim(),
        description: formData.description.trim(),
        content: formData.content.trim(),
        category_id: Number(formData.category_id),
        ...(formData.article_id && { article_id: Number(formData.article_id) })
      }

      const revision = await RevisionService.createRevision(data)
      ...
      setError(err.message || '修正案の作成に失敗しました')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">新規修正案作成</h1>
            <p className="text-gray-600 mt-2">ナレッジの修正案を作成します</p>
          </div>
        </div>
