...
import { getRevisions, RevisionsListParams } from '@/lib/api/revisions';
...
export default function RevisionsPage() {
  ...
  const fetchRevisions = useCallback(async (params: RevisionsListParams = {}) => {
    try {
      setLoading(true);
      const response: PaginatedResponse<Revision> = await getRevisions({
  ...
  }, [currentPage, statusFilter]); // toastを依存配列から削除
  ...
  const columns = [
    ...
    {
      key: 'proposer_name' as keyof Revision,
      title: '提案者',
      width: '120px'
    },
    {
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-semibold text-white">修正案一覧</h1>
            <p className="text-gray-400 mt-1">システム内の全ての修正案を管理します</p>
          </div>
